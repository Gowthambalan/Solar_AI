...
class InverterDataProcessor:
    ...
    def process_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def validate(section, key, current, prev):
            """Validate and fix invalid numeric values; handle lists safely."""
            if section not in validation_rules or key not in validation_rules[section]:
                return current

            # Handle list or non-numeric cases
            if isinstance(current, list):
                if len(current) > 0:
                    current = current[0]
                else:
                    anomalies.append(f"{section}.{key} empty list value replaced with previous {prev}")
                    return prev

            min_val, max_val = validation_rules[section][key]

            # Handle non-numeric or None
            if current is None or not isinstance(current, (int, float)):
                anomalies.append(f"{section}.{key} non-numeric value {current} replaced with previous {prev}")
                return prev
        ...
        # Log anomalies externally
        reading_id = (
            record["properties"].get("readingId", {}).get("value")
            or record["properties"].get("asset", {}).get("properties", {}).get("inverterId", {}).get("value")
            or "UNKNOWN"
        )
        self.log_anomalies(reading_id, anomalies)
